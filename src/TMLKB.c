    ...
    *partName = '(';
    ...
    do {
        *comma = '\0';
        ...
        if (odd == 1) {
            if (end == 0) {
                sprintf(ret_odd, "%s%s,", ret_even,
                        (part->name == NULL || strchr(part->name, '.') != NULL) ? partName : part->name);
            } else {
                sprintf(ret_odd, "%s%s)", ret_even,
                        (part->name == NULL || strchr(part->name, '.') != NULL) ? partName : part->name);
            }
 ...
 */
char *createNormalizedRelStr(TMLKB *kb, char *relation, char *object, Node **argNodes, int nargs, int addBase,
                             int useNames) {
    int a;
    char ret_even[MAX_LINE_LENGTH + 1];
    char ret_odd[MAX_LINE_LENGTH + 1];
 ...
 */
char *splitRelArgsAndCreateNormalizedRelStr(TMLKB *kb, char *relStr, char *relation, int *nargs, char ***args,
                                            int useNames) {
    int n = 0;
    char obj_fmt_str[25];
    char *iter = relStr;
 ...
 */
Node *updateClassForNodeRecHelper(TMLKB *kb, KBEdit **editPtr, char *name, Node *topNode, TMLClass *cl,
                                  TMLClass *finecl, FILE *tmlFactFile, int linenum) {
    int i, j;
    Node *ret = NULL;
    Node *obj;
    Node *par;
 ...
 * @return pointer for the object:subcl node
 */
Node *blockClassForNodeRecHelper(TMLKB *kb, KBEdit **editPtr, char *name, Node *topNode, TMLClass *cl, TMLClass *finecl,
 ...
 */
Node *updateClassForNode(TMLKB *kb, KBEdit **editPtr, char *name, Node *node, TMLClass *cl, FILE *tmlFactFile,
                         int linenum) {
    int i;
 ...
 * @return node for this object with the coarsest type information
 */
Node *blockClassForNode(TMLKB *kb, KBEdit **editPtr, char *name, Node *node, TMLClass *cl, FILE *tmlFactFile,
                        int linenum) {
    int i;
    TMLClass *parcl = cl->par;
    Node *initNode = node;
 ...
 */
void addAndInitSubpartRecHelper(TMLKB *kb, Node *par, Node *obj, Node *subpart, char *part, int n, FILE *tmlFactFile,
                                int linenum) {
    int i, j, c;
    TMLPart *foundPart;
    TMLClass *cl = obj->cl;
 ...
 */
Node *addAndInitSubpart(TMLKB *kb, char *name, char *subpartname, Node *obj, char *part, int n, FILE *tmlFactFile,
                        int linenum) {
    TMLClass *partcl;
    int i, j, k;
    int found = 0;
...
int readInObjectClasses(TMLKB *kb, Node *node, FILE *tmlFactFile, int linenum, char *line) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((i == n - 1 && correctScan != 1) || (i != n - 1 && correctScan != 2) ||
        ...
        if (className[0] == '!') {
            output = addClassEvidenceForObj(kb, (node->name == NULL ? node->pathname : node->name), node, className + 1,
                                            0, tmlFactFile, linenum);
        } else {
            output = addClassEvidenceForObj(kb, (node->name == NULL ? node->pathname : node->name), node, className, 1,
                                            tmlFactFile, linenum);
        }
...
int readInRelations(TMLKB *kb, TMLClass *cl, char *line) {
    ...
    snprintf(rel_hard_fmt_str, 60, " %%%d[!a-zA-Z0-9:] (%%%d[] a-zA-Z0-9:[,] %%1[)] %%1s", MAX_NAME_LENGTH,
             MAX_LINE_LENGTH);
    ...
    for (r = 0; r < n; r++) {
                ...
                for (a = 0; a < nargs; a++) {
                    ...
                    if (part != NULL) {
                        ...
                        while (tempcl != NULL) {
                            ...
                            if (part != NULL) {
                                if (part->defaultPart == 0 || (part->defaultPartForSubcl != NULL &&
                                                               part->defaultPartForSubcl[tempcl2->subclIdx] == 0)) {
        ...
        if (pol == 0) {
            if (hard == 1) {
                ...
                rel->pwt = log(0.00001);
                if (rel->hard == 1) {
                    printf("Error in relations for class %s: Rule %s is already defined as being always negative.",
                           cl->name, relName);
                    return 0;
                }
                rel->hard = -1;
 ...
 */
int readInOneTMLClass(TMLKB *kb, const char *className, TMLClass **rootCl, FILE *tmlRuleFile, int linenum, int id,
                      int counts, int first) {
    char *line;
 ...
 */
float computeLogZ(Node *node, TMLClass *assignedClassBySuperpart, float(*spn_func)(float *arr, int num, int *idx),
                  int recompute) {
    int i, j, k;
    int c;
    float logZ = 0.0;
    ...
    if (node->assignedSubcl != -1 && cl->nsubcls != 0) {
        ...
        if (node->subclMask == NULL)
            ...
            logZ += cl->wt[assignedSubcl] +
                    computeLogZ(&(node->subcl[assignedSubcl]), assignedClassBySuperpart, spn_func, recompute);
...
float fillOutSPN(TMLKB *kb, Node *node, TMLClass *assignedClassBySuperpart, char *anonName) {
    ...
    if (node->assignedSubcl == -1 && cl->nsubcls != 0 && node->subclMask == NULL) {
        ...
        if (node->subclMask == NULL)
            ...
            logZ += cl->wt[node->assignedSubcl] +
                    fillOutSPN(kb, &(node->subcl[node->assignedSubcl]), assignedClassBySuperpart, anonName);
    } else {
        i = 0;
        HASH_ITER(hh, cl->rel, rel, tmp) {
            logZ += relWeight((*relValues), rel);
...
int determineObjectLineType(TMLKB *kb, TMLClass *cl, char *line) {
    ...
    if (correctScan == 2) {
        ...
        if (str1[0] == '!') HASH_FIND_STR(kb->classNameToPtr, str1 + 1, tmpcl);
            ...
            HASH_FIND_STR(kb->classNameToPtr, str1, tmpcl);
...
int blockClassesForPartQueryRec(KBEdit **editsPtr, Node *node, TMLPart *part, int n, TMLClass *newClass) {
    ...
    if (foundPart != NULL) {
        ...
        if (newClass != NULL && !isAncestor(foundPart->cl, newClass) &&
            !isAncestor(newClass, foundPart->cl))
            blockDefault = 1;
        if (foundPart->defaultPart == 0) {
            return blockDefault;
        }
        if (node->subclMask != NULL) {
            for (c = 0; c < cl->nsubcls; c++) {
                ...
                block = blockClassesForPartQueryRec((editsPtr != NULL ? &edits : NULL), &(node->subcl[c]), part, n,
                                                    newClass);
                if (block == 1) {
                    if (node->subclMask[c] == 1) node->subclMask[c] = -1;
                    else if (node->subclMask[c] < 0) node->subclMask[c]--;
...
void computeAttributeQueryOrAddEvidenceForObj(TMLKB *kb, Node *obj, TMLAttribute *attr, TMLAttrValue *attrval, int pol,
                                              float logZ, int isQuery, int isMultQuery, FILE *outFile) {
    char *best = NULL;
    TMLClass *cl;
    float newLogZ;
    char *name;
        ...
        if (obj->assignedAttr[attr->idx] != NULL) {
            if (obj->assignedAttr[attr->idx] == attrval) {
                ...
                if (isQuery) {
                    ...
                    if (outFile != NULL)
                        fprintf(outFile,
                                "%s is already the assigned value of attribute %s for object %s. Attribute values are mutually exclusive.\n",
                                obj->assignedAttr[attr->idx]->name, attr->name, name);
                }
            }
            if (best != NULL) free(best);
...
void computeRelationQueryOrAddEvidence(TMLKB *kb, char *relName, char *rest, int pol, float logZ, int isQuery,
                                       FILE *outFile) {
    ...
    if (node == NULL) {
        ...
        while (qnode != NULL) {
            ...
            if (tmp->name != NULL) {
                ...
                computeRelationQueryOrAddEvidenceForObj(kb, node, tmp->name, relName, iter, pol, normalizedRelStr, args,
                                                        p, logZ, isQuery, 1, outputRelStr, outFile);
            } else {
                HASH_FIND(hh_path, kb->objectPathToPtr, tmp->pathname, strlen(tmp->pathname), node);
                computeRelationQueryOrAddEvidenceForObj(kb, node, tmp->pathname, relName, iter, pol, normalizedRelStr,
                                                        args, p, logZ, isQuery, 1, outputRelStr, outFile);
            }
            qnode = qnode->next;
        }
        for (n = 0; n < p; n++)
    ...
    computeRelationQueryOrAddEvidenceForObj(kb, node, base, relName, iter, pol, normalizedRelStr, args, p, logZ,
                                            isQuery, 0, outputRelStr, outFile);
    for (n = 0; n < p; n++)
        free(args[n]);
    free(args);
    free(normalizedRelStr);
...
void computeRelationQueryOrAddEvidenceForObj(TMLKB *kb, Node *topNode, char *base, char *relName, char *iter, int pol,
                                             char *normalizedRelStr, char **args, int p, float logZ, int isQuery,
                                             int isClassQuery, char *outputRelStr, FILE *outFile) {
    int i, j, k, n;
    Node ***argNodes;
    int *argLen;
    Node **currArgNodes;
            ...
            for (i = 0; i < p; i++) {
                if (currArgNodes[i]->npars == 0)
                    ...
                    for (j = 0; j < currArgNodes[i]->npars; j++) {
                        argParNodes[i][j] = blockClassesForPartQuery(&queryEdits, currArgNodes[i]->par[j],
                                                                     currArgNodes[i], NULL);
                    }
                }
            }
        ...
        for (i = 0; i < p; i++) {
            if (argNodes[i][0]->npars == 0)
                ...
                for (j = 0; j < argNodes[i][0]->npars; j++) {
                    if (isQuery)
                        argParNodes[i][j] = blockClassesForPartQuery(&queryEdits, argNodes[i][0]->par[j],
                                                                     argNodes[i][0], NULL);
                    else
                        argParNodes[i][j] = blockClassesForPartQuery(&(kb->edits), argNodes[i][0]->par[j],
                                                                     argNodes[i][0], NULL);
                }
            }
        }
...
void computeClassQueryForObject(TMLKB *kb, const char *objName, Node *obj, const char *clName, TMLClass *cl, float logZ,
                                FILE *outFile) {
    Node *node;
    Node *prevFinest;
    int subcl;
    ...
    propagateKBChange(node);
    newLogZ = computeLogZ((Node *) (kb->root->ptr), ((Node *) (kb->root->ptr))->cl, spn_logsum,
                          (kb->mapSet == 1) ? 1 : 0);
...
float computeQueryOrAddEvidence(TMLKB *kb, char *query, float logZ, int isQuery, const char *output) {
    ...
    snprintf(has_fmt_str, 60, " Has ( %%%d[^, \t\r\n\v\f] , %%%d[^, \t\r\n\v\f] , %%%d[^, \t\r\n\v\f] %%1s",
             MAX_NAME_LENGTH, MAX_NAME_LENGTH, MAX_NAME_LENGTH);
    ...
    if (correctScan == 3) { // subpart fact / query
        ...
        if (subObj != NULL) {
            ...
            propagateKBChange(obj);
            return computeLogZ((Node *) (kb->root->ptr), ((Node *) (kb->root->ptr))->cl, spn_logsum,
                               (kb->mapSet == 1) ? 1 : 0);
        }
    }
        ...
        if (attr != NULL) {
            if (correctScan == 2) { // check all values
                ...
                while (qnode != NULL) {
                    ...
                    if (tmp->name != NULL) {
                        HASH_FIND_STR(kb->objectNameToPtr, tmp->name, obj);
                        computeAttributeQueryOrAddEvidenceForObj(kb, obj, attr, attrval, pol, logZ, isQuery, 1,
                                                                 outFile);
                    } else {
                        HASH_FIND(hh_path, kb->objectPathToPtr, tmp->pathname, strlen(tmp->pathname), obj);
                        computeAttributeQueryOrAddEvidenceForObj(kb, obj, attr, attrval, pol, logZ, isQuery, 1,
        ...
        if (rel->defaultRel == 0 || rel->defaultRelForSubcl[nextSubcl] == 0) {
            if (node->relValues[r][2] != 0 || rel->hard != 0) {
                if (rel->nargs != 0) {
                    ...
                    for (c = 0; c < ncombo; c++) {
                        currArgNodes = (Node **) nextArraysAccessor(aa);
                        normalizedGroundStr = createNormalizedRelStr(kb, rel->name, node->pathname, currArgNodes,
                                                                     rel->nargs, 0, 0);
        ...
        if (rel->defaultRel == 0 || rel->defaultRelForSubcl[nextSubcl] == 0) {
            if (node->relValues[r][2] != 0) {
                if (rel->nargs != 0) {
                    ...
                    for (c = 0; c < ncombo; c++) {
                        currArgNodes = (Node **) nextArraysAccessor(aa);
                        normalizedGroundStr = createNormalizedRelStr(kb, rel->name, name, currArgNodes, rel->nargs, 0,
                                                                     0);
                        outputGroundStr = createNormalizedRelStr(kb, rel->name, node->name, currArgNodes, rel->nargs, 1,
                                                                 1);
                        if (objRelHash != NULL) {
                            HASH_FIND_STR(objRelHash->hash, normalizedGroundStr, relStrHash);
                            if (relStrHash != NULL) continue;
